    ...
    if not state then
        ...
        Screen.beforePaint = function(screen, ...)
            ...
            if first_paint and not state.bypass and enabled and owner._pageturnanimation_pending_direction then
                ...
                logger.info("PageTurnAnimation: armed KPW4 reveal, direction", state.direction)
            end
            ...
            return function(screen, ...)
                ...
                if ok then
                    ...
                    if config.full_refresh and screen.refreshFull then
                        ...
                        -- A2. This is deliberately stronger than the normal
                        -- AUTO/UI settle and may visibly flash.
                        screen:refreshFull(0, 0, w, h)
                    elseif screen.refreshUI then
                        -- Default behavior: one full-screen UI/AUTO settle
                        -- after the six reveal updates.
                    ...
                    logger.warn("PageTurnAnimation page-turn animation failed:", result)
                    restoreDestination(screen, new_bb)
                    state.suppress = false
                    original(screen, ...)
                end
